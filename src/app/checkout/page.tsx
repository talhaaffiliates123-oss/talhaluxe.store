...
function CheckoutForm({ clientSecret }: { clientSecret: string }) {
    ...
    const handlePlaceOrder = async (e: React.FormEvent) => {
      ...
      try {
          ...
          newOrderRef = doc(ordersCollection);
          ...
          // Trigger confirmation email via extension
          const mailCollection = collection(firestore, 'mail');
          addDoc(mailCollection, {
              to: [orderData.shippingInfo.email],
              message: {
                  subject: `Your Talha Luxe Order Confirmation #${newOrderRef.id.substring(0,8)}`,
                  html: `<h1>Thanks for your order, ${orderData.shippingInfo.name}!</h1><p>We're getting your order ready to be shipped. Your Order ID is ${newOrderRef.id}.</p>`,
              },
